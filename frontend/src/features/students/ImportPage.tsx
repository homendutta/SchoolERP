/* Student Import — migration mode. Upload → Validate → Preview → Import →
 * Summary. Validates duplicate admission numbers, missing guardian, invalid
...
export function ImportPage() {
  ...
  const [text, setText] = useState(
    'admission_number,name,gender,guardian_name,guardian_phone,academic_year_id,class_id,section_id\n9001,Imported Kid,male,Parent Name,9876500000,1,1,1'
  );
  ...
  const validate = async () => {
    ...
    setRows(parsed);
    setSummary(null);
    const res = await studentsApi.import.validate(parsed);
    setValid(res.valid);
  ...
  const execute = async () => {
    ...
    const res = await studentsApi.import.execute(parsed);
    setSummary(res);
    setValid(null);
  };
  ...
  const columns: AXColumn<Row>[] = [
    ...
    { key: 'admission_number', header: 'Adm. No.', render: (r) => r.admission_number },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-[var(--navy-primary)]">Student Import</h2>
        <AXBadge tone="amber">Migration mode</AXBadge>
      </div>
