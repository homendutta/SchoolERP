/* Staff Documents — upload through the shared Media pipeline (media_id only).
 * Document type comes from Master Data. */
...
export function DocumentsPage() {
  ...
  const docTypes = useMasterValues('staff_document_types');
  ...
  const load = () => {
    if (id) staffApi.documents.list({ filter: { staff_id: id } }).then((r) => setDocs(r.data));
  };
  useEffect(() => {
    setDocs([]);
    if (id) load();
  ...
  const onFile = async (file: File | null) => {
    ...
    try {
      ...
      await staffApi.documents.create({
  ...
  const columns: AXColumn<StaffDoc>[] = [
    {
      key: 'title',
      header: 'Title',
      render: (r) => (
    ...
    },
    {
      ...
      render: (r) => (
        <button
          onClick={() =>
            ask('Remove this document?', () => staffApi.documents.archive(r.id).then(load))
          }
